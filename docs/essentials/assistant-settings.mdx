...
InboxJarvis learns when senders or email types consistently match the same rule. Open **Learned patterns** to view, edit, or remove those associations. Remove a pattern when it starts routing a sender incorrectly.

## Advanced settings

### Sync to browser extension

Sync label-based assistant rules to a compatible upstream Inbox Zero Tabs extension as Gmail tabs. This requires a compatible configured extension and a Chromium browser, and it applies to Gmail rather than Outlook. Select the label rules you want before starting the sync.
...
Optionally append an InboxJarvis referral link to generated drafts.
...
Some settings depend on your plan or email provider. Auto-draft controls appear only while auto-drafting is enabled, Digest access depends on your deployment’s billing configuration, and extension sync requires a compatible upstream Inbox Zero Tabs extension.
