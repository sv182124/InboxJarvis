...
description: 'Chat with your assistant and receive InboxJarvis updates in Telegram.'
icon: 'paper-plane'
---

Connect Telegram from the unified **Channels** page. The integration uses a direct message with the InboxJarvis bot for chat and feature delivery.

## Connect Telegram

1. Open **Channels** in the left sidebar.
2. Find Telegram and click **Connect**.
3. InboxJarvis generates a one-time `/connect` command.
4. Open the InboxJarvis bot using the link in the dialog.
...
- `/connect <code>`: Link an InboxJarvis email account.
