...
description: "Set up the Teams bot for InboxJarvis assistant chat."
---

This guide is for self-hosted deployments that want to enable Teams chat with the InboxJarvis assistant.

## What Teams currently supports

- AI assistant chat in **direct messages** with the InboxJarvis bot
...
- One-click draft editing and sending inside Teams; review or send those drafts in InboxJarvis
...
```json
{
  ...
  "name": {
    "short": "InboxJarvis",
    "full": "InboxJarvis Assistant"
  },
  "description": {
    "short": "Manage email with InboxJarvis for Microsoft Teams",
    "full": "Manage email with your InboxJarvis AI assistant in Microsoft Teams. An active InboxJarvis account is required. Sign up or get started at https://your-domain.com. For help, visit https://your-domain.com/docs/essentials/channels or contact support@your-domain.com. AI-generated content may be inaccurate and should be reviewed before use. Report objectionable AI-generated content to support@your-domain.com."
  },
  "icons": {
    "outline": "outline.png",
    "color": "color.png"
      ...
      "commandLists": [
        {
          "scopes": ["personal"],
          "commands": [
            { "title": "connect", "description": "Link an InboxJarvis account with a connect code" },
            { "title": "switch", "description": "List or switch linked InboxJarvis accounts" },
...
## 7. Connect a user account from InboxJarvis

Each InboxJarvis email account links to a Teams user via a connect code:

1. In InboxJarvis, go to **Settings** → **Connected Apps**
2. Click **Connect Teams**
3. Copy the generated command: `/connect <code>`
4. Open a DM with the InboxJarvis bot in Teams
5. Send the command
...
```text
Use Azure CLI to set up the Microsoft Teams integration for InboxJarvis. Create a single-tenant Entra app registration with a client secret, create an Azure Bot resource on the F0 tier, point it at https://<your-domain>/api/teams/events, enable the Teams channel, and then tell me exactly which environment variables and manual Azure/Teams UI steps are left for me to complete.
```
