...
4. **Update [scopes](https://console.cloud.google.com/auth/scopes):**
   ...
   4. Add only the optional feature scopes your deployment will use:
      ...
      **Contacts** — only when `NEXT_PUBLIC_CONTACTS_ENABLED=true`:
      ...
      Existing accounts can continue using core email features without this
      scope. When they open the composer, InboxJarvis offers an optional
      reconnect action if they want contact suggestions.
      ...
      **Google Drive, Standard access** — lets InboxJarvis create folders and access files it created or the user explicitly opened with it:
