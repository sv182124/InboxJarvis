...
InboxJarvis can proxy remote email images and CSS-loaded assets so senders do not see the end user's IP address when an email is rendered.

This is optional, but recommended if you host InboxJarvis for other people.
...
The repo includes a ready-to-deploy Worker in [apps/image-proxy](https://github.com/sv182124/InboxJarvis/tree/main/apps/image-proxy).
...
### 5. Configure the InboxJarvis App
...
If you set `NEXT_PUBLIC_IMAGE_PROXY_BASE_URL` without `IMAGE_PROXY_SIGNING_SECRET`, InboxJarvis emits unsigned `?u=` proxy URLs.
...
If you do not want Cloudflare, the repo also includes an AWS Lambda adapter in [apps/image-proxy-aws](https://github.com/sv182124/InboxJarvis/tree/main/apps/image-proxy-aws).
...
If you are self-hosting and want the simplest setup, InboxJarvis can proxy remote images through its own Next.js app route instead of a separate service.
...
In this mode, InboxJarvis signs image proxy URLs and routes them to `${NEXT_PUBLIC_BASE_URL}/api/image-proxy`.
