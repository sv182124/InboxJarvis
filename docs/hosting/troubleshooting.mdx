...
description: 'Solutions for common issues when running InboxJarvis'
---
...
- **Slow page loads with `validate-email-account` or `Unable to parse response body` Redis warnings**: `docker compose` may have started `serverless-redis-http` without the token from `apps/web/.env`
  - Solution: make sure repo-root `.env` matches `apps/web/.env` before starting Docker, create a link if no root `.env` exists: `ln -s apps/web/.env .env`. Preserve and review any existing root configuration, then recreate the affected containers with `docker compose --env-file apps/web/.env --profile all up -d`
...
1. Check [GitHub Issues](https://github.com/sv182124/InboxJarvis/issues) for similar problems
2. Open an issue with the relevant logs and steps to reproduce
