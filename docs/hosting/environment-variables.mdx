...
description: 'Reference for self-hosting environment variables used in InboxJarvis'
---

Reference for environment variables relevant to self-hosting InboxJarvis. Hosted-only billing, analytics, and internal operations variables are intentionally omitted unless they affect common self-hosted deployments.
...
| `NEXT_PUBLIC_SLACK_BOT_NAME` | No | Bot display name shown in the app | `InboxJarvis` |
| **Messaging Adapters** ||||
...
| **Redis** ||||
| `REDIS_HTTP_URL` | No* | HTTP Redis endpoint for caching and rate limits. Accepts Upstash, Vercel KV, or your own [serverless-redis-http](https://github.com/hiett/serverless-redis-http) proxy (*required if not using Docker Compose with local Redis) | — |
| `REDIS_HTTP_TOKEN` | No* | Token for `REDIS_HTTP_URL` (*required if not using Docker Compose) | — |
| `REDIS_URL` | No | Redis URL for subscriptions and the optional BullMQ worker | — |
| `UPSTASH_REDIS_URL` | Deprecated | Legacy alias for `REDIS_HTTP_URL`. Still read when `REDIS_HTTP_URL` is unset | — |
| `UPSTASH_REDIS_TOKEN` | Deprecated | Legacy alias for `REDIS_HTTP_TOKEN`. Still read when `REDIS_HTTP_TOKEN` is unset | — |
...
| `RESEND_FROM_EMAIL` | No | From email address | Configure a verified sender, e.g. `InboxJarvis <verified-sender@example.com>` |
...
| `UNSUBSCRIBE_WORKER_URL` | No | HTTPS URL of an optional isolated browser unsubscribe worker. Leave unset to keep the existing HTTP unsubscribe flow. Docker Compose does not start this worker. | — |
| `UNSUBSCRIBE_WORKER_SECRET` | No* | Shared secret (at least 32 characters). Required when `UNSUBSCRIBE_WORKER_URL` is set. | — |
...
| **Feature Flags** ||||
| `NEXT_PUBLIC_CONTACTS_ENABLED` | No | Enable Gmail and Outlook contact suggestions while composing | `false` |
| `NEXT_PUBLIC_EMAIL_SEND_ENABLED` | No | Enable email sending | `true` |
| `NEXT_PUBLIC_EXTERNAL_API_ENABLED` | No | Enable external API endpoints, API keys, and API key UI. Also set `API_KEY_SALT`. Required for the MCP server. | `false` |
| `MCP_SERVER_ENABLED` | No | Enable the remote OAuth MCP server at `/mcp`. Requires `NEXT_PUBLIC_EXTERNAL_API_ENABLED=true`. | `false` |
...
| `NEXT_PUBLIC_TABS_EXTENSION_ID` | No | Chrome extension ID used for compatible upstream Tabs sync | Built-in extension ID |
| **White Labeling (Optional)** ||||
| `NEXT_PUBLIC_BRAND_NAME` | No | Brand name used in UI text and metadata | `InboxJarvis` |
| `NEXT_PUBLIC_BRAND_LOGO_URL` | No | Custom logo URL or public asset path (for example `/images/brand-logo.svg`) | Built-in InboxJarvis logo |
| `NEXT_PUBLIC_BRAND_ICON_URL` | No | Custom app icon URL or public asset path | `/icon.png` |
| `NEXT_PUBLIC_SUPPORT_EMAIL` | No | Contact email shown in support links and error messages | Unset; support links use the repository issues page |
| **Debugging** ||||
