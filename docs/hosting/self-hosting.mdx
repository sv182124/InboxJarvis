...
Use the setup CLI from the InboxJarvis checkout:
...
```bash
pnpm exec tsx packages/cli/src/main.ts setup-google --project-id YOUR_PROJECT_ID --domain yourdomain.com
```

Run this command from the repository root after installing dependencies.

The command enables required APIs, creates the Pub/Sub topic and subscription, and guides you through OAuth credential creation. It prints environment variables for you to add to your `.env`; it uses the source checkout.
...
The default prebuilt image, `ghcr.io/elie222/inbox-zero:latest`, is published by upstream Inbox Zero. It does not contain InboxJarvis source changes. To run this checkout, follow **Building from Source** below and use your locally built image.
...
- **External Redis**: Set `REDIS_HTTP_URL` and `REDIS_HTTP_TOKEN`. `UPSTASH_REDIS_URL` and `UPSTASH_REDIS_TOKEN` still work.
...
InboxJarvis supports multiple background job backends for self-hosted deployments:
...
Build this checkout to include the InboxJarvis changes:
...
docker build -f docker/Dockerfile.local -t inboxjarvis:local .
```

Create `docker-compose.inboxjarvis.yml` to select your locally built image:

```yaml
services:
  web:
    image: inboxjarvis:local
    pull_policy: never
  worker:
    image: inboxjarvis:local
    pull_policy: never
...
Start the stack with that override:

```bash
NEXT_PUBLIC_BASE_URL=https://yourdomain.com docker compose --env-file apps/web/.env -f docker-compose.yml -f docker-compose.inboxjarvis.yml --profile all up -d
```
