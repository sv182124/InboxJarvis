...
description: 'Run InboxJarvis on Kubernetes with the Helm chart'
---

<Info>
Build and publish an image from this checkout to deploy InboxJarvis changes. Existing `ghcr.io/elie222/inbox-zero` images are upstream Inbox Zero releases; they are retained as compatibility defaults and are not InboxJarvis releases.
</Info>

InboxJarvis can run on Kubernetes as the same pieces used by Docker Compose:
...
`inbox-zero-redis` should contain `REDIS_URL`, `REDIS_HTTP_URL`, and `REDIS_HTTP_TOKEN`. Secrets that still use `UPSTASH_REDIS_URL` and `UPSTASH_REDIS_TOKEN` keep working: leave `redisHttpUrlKey` and `redisHttpTokenKey` empty, or point those keys at the legacy names.
