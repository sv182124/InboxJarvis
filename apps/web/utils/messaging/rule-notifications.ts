...
function getLinkedProviderLimitationText({
  ...
  if (isDraftReplyActionType(actionType)) {
    return provider === MessagingProvider.TEAMS
      ? `One-click draft editing and sending aren't available in Teams yet. Use ${BRAND_NAME} to review or send this draft.`
      : `Draft editing isn't available in Telegram yet. You can send this draft from Telegram or use ${BRAND_NAME} to revise it first.`;
  }
